...
+
  $ aws ssm put-parameter --name GREETING --value Hello --type SecureString 
  {
      "Version": 1
  }
+
This will create a secret in the Parameter Store using the default KMS key. Optionally, a new key may be created using `aws kms create-key` and then specified using the option `--key-id <key-ARN>`.
+
...
$ aws ssm get-parameter --name GREETING
{
    "Parameter": {
        ...
        "Type": "SecureString", 
        "Name": "GREETING", 
        "Value": "AQICAHjpQ3N/3+MWjuLpKf2zXN8hhDD7QhU4T1DSp3yvrOl4nwH2WebRE/HkN0YbwrCdIuW8AAAAYzBhBgkqhkiG9w0BBwagVDBSAgEAME0GCSqGSIb3DQEHATAeBglghkgBZQMEAS4wEQQMmsXjDFq043rG9vXsAgEQgCBXL51EEG7Ksn64XtO2lIHCX5sOnuJBhs9bpye5yhbzgQ=="
    }
}
```
+
...
. Create another secret:
  ...
  $ aws ssm put-parameter --name NAME --value World --type SecureString 
  {
      "Version": 1
  }
+
