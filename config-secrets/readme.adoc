...

  {
    ...
    "Statement": [
      ...
      },
      {
        ...
        "Principal": {
          "AWS": [
            "arn:aws:iam::123456789012:user/arun",
            "arn:aws:iam::123456789012:role/nodes.example.cluster.k8s.local"
          ]
        },
        "Action": [
          "kms:Create*",
...
The Kubernetes EC2 worker nodes use an instance profile to allow the EC2 instances to access other AWS services. This role
must be updated to allow the worker nodes to read the secrets from Parameter Store.
...

    {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    "arn:aws:ssm:us-east-1:295744685835:parameter/GREETING",
                    "arn:aws:ssm:us-east-1:295744685835:parameter/NAME"
                ]
            }
        ]
    }
...
+
  $ aws ssm put-parameter \
    ...
    --value Hello \
    --type SecureString \
    --key-id arn:aws:kms:us-east-1:123456789012:key/414a963b-7fe4-4a61-b19f-ea408b9bda3b
  {
      "Version": 1
  }
+
