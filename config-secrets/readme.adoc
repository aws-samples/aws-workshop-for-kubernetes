...
. Read the service account token needed by Vault:
...
. Copy the certificate from S3 bucket:

  aws s3 cp $KOPS_STATE_STORE/example.cluster.k8s.local/pki/issued/ca/*.crt ca.crt
