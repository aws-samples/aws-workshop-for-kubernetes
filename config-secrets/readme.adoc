...
. Check that this value can be read:
  ...
  [ec2-user@ip-172-31-26-180 ~]$ ./vault read -field=GREETING secret/creds
  Hello
+
This can also be verified by downloading the Vault CLI on your local machine:
+
```
...
```
+
The value of token is printed during server startup. It is also stored in the file `~/.vault-token` where the server was started.

=== Deploy a Pod using secrets from Vault

Let's deploy a Pod that is reading secrets from the Vault server.
...
. Get the list of Pods:
  ...
  NAME               READY     STATUS      RESTARTS   AGE
  java-app-secrets   0/1       Completed   0          20s

. Get logs from the complete Pod:

  $ kubectl logs java-app-secrets
