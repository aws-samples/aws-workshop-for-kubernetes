...
Check the Istio has been installed
...
```
If all components were installed successfully, you should be able to see the product page
...
The Envoy proxy does not round robin the routing to different versions of the microservice, but over
a hundred requests 50% of them should be routed to a different version. We can test this using
the script below. Make sure you don't have a file called 'mfile' in your current folder before running this.
The 100 'curls' may take around 30s. For the eagle eyed amongst you, the divde by 2 is because the
...
You should see a distributed trace that looks something like this
