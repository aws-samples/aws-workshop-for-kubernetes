...
== Create hosted zone on Route53 (optional)

As of Kops 1.6.2, a top-level domain or a subdomain is no longer required to create the cluster. Instead, a gossip-based cluster can be easily created. The only requirement to trigger this is to have the cluster name end with `.k8s.local`. If you wish to use a gossip-based cluster, then you can skip this section.
...
Create a simple Kubernetes cluster using the following command. The cluster name suffix of `.k8s.local` will result in the gossip protocol being used for masters/nodes to discover each other. Note that 'create cluster' only creates and stores the cluster config in S3; you must use `update cluster` to build the cluster:
