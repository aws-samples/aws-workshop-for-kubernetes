...
To create a DNS-based Kubernetes cluster you'll need a top-level domain or subdomain that meet one of the following scenarios:
...
=== Creating a gossip-based cluster
...
A multi-master cluster can be created by using the `--master-count` option and using an odd number value. The list of master zones can be defaulted from the value of the `--zones` option. Alternatively, the AZs for master can be specified using the `--master-zones` option. Kops will spread the servers across different AZs.
