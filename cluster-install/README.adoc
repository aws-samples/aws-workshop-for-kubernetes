...
Create a simple Kubernetes cluster using the following command. The cluster name suffix of '.k8s.local' will result in the gossip protocol being used for masters/nodes to discover each other. Note that 'create cluster' only creates and stores the cluster config in S3; you must use 'update config' to build the cluster:
