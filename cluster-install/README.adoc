...
The cluster name suffix of `.k8s.local` will result in the gossip protocol being used for master/node discovery. Note that `create cluster` only creates and stores the cluster config in S3. The `update cluster` command is used with `--yes` option to create all the resources required for the cluster.

Alternatively, `--yes` option can be specified with `create cluster` command to immediately create the cluster as well.

By default, this will create a single master node and two worker node cluster.
