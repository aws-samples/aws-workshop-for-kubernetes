...
A multi-master cluster can be created by using the `--master-count` option and specifying an odd number value. By default, the master nodes are spread across the AZs specified using the `--zones` option. Alternatively, `--master-zones` option can be used to explicitly specify the zones for master.
...
A multi-master cluster can be created by using the `--master-count` option and using an odd number value. By default, master nodes are spread across the AZs specified using `--zones`. Alternatively, `--master-zones` option can be used to explicitly specify the zones for master.
