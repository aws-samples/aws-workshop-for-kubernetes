...
If you already have kops for MacOS, then make sure you have the latest version:
...
If you created a private VPC, then an additional cleanup of resources is required as shown below:
    ...
    aws ec2 detach-internet-gateway --internet $IGW --vpc $VPCID --region $AWS_DEFAULT_REGION
