...
Limits can be seen using the following command:

	$ kubectl get pod/nginx-pod -o jsonpath={.spec.containers[].limits}

This command shows an empty output.

===== Assign memory and CPU

Let's assign a memory request and limit to a Pod:
...
When every Container in a Pod is given a memory and CPU request and limit, and they exactly match, then a Pod is scheduled with `Guaranteed` QoS. This is the higest priority.

A Pod is given `Burstable` QoS class if the Pod does not meet the `Guaranteed` QoS and at least one Container has a memory or CPU request.

When no memory or CPU request or limit is assigned to any Container in the Pod, then a Pod is scheduled with `BestEffort` QoS. This the lowest priority.
