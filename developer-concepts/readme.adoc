...
Only one pod is started, unless the pod fails. Job is complete as soon as the pod terminates successfully.
...
To begin with, it shows that the pod for the job is running. The pod successfully exits after a few seconds.
...
The list of completed pods can be seen using the following command:
...
This is also called as Fixed Completion Count job.
...
`batch/v2alpha1` API version for Cron Job is deprecated for cluster version >= 1.8. You should switch to `batch/v1beta1`. This version is enabled by default in the API server.
