...
This section will explain how to use http://jaeger.readthedocs.io/en/latest/[Jaeger] to perform distribute tracing on applications deployed on Kubernetes.
...
We will use the Istio service mesh sample application, so follow these steps: link:../service-mesh#install-istio[install istio] and link:../service-mesh#deploy-the-sample-application[deploy the sample application]
...
image::images/jaeger-traces, 800]
