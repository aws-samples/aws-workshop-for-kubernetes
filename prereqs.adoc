...
If you already have an AWS Account, you need to create an IAM user to use during the workshop. Make sure you have run `aws configure` with your personal `Access Key ID` and `Secret Access Key`.
After that run the follwing commands in you commandline to create the needed group and user:
...
Several parts of the workshop require the region or availability zones to be explicitly specified as a CLI option. The environment variable `$AWS_AVAILABILITY_ZONES` is used to set the availability zones.
