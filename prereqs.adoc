...
To obtain your personal access keys, go to https://console.aws.amazon.com/iam/home#/security_credential, click on `Access Keys` -> `Create New Access Key`. In the resulting popup click on `Show Access Key` to see your credentials once. Finally copy and pass them to the `aws configure` command. 
