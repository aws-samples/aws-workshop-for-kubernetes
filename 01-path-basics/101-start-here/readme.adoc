...
At this point you can terminate the Cloud9 IDE terminal session and start a new terminal session by going to `Window`, `New Terminal` to ensure that the kublectl completion is enabled. Once a new terminal window is opened, type $ kubectl get nodes.
