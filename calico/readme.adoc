...
This will create a 1 master node and 3 worker nodes cluster. It uses `--networking calico` which tells the cluster to use calico networking instead of the default networking provided by kubenet. Note, the name here is `cluster2.k8s.local` instead of the usual `cluster.k8s.local` name. This is just to make sure, in case, the two clusters can coexist.
...
+
```
/ # wget -S http://http-echo.ns-1.svc.cluster.local/test --post-data '{"message":"hello"}'
...
```
/ # wget -S http://http-echo.ns-1.svc.cluster.local/test2 --post-data '{"message":"hello"}'
Connecting to http-echo.ns-1.svc.cluster.local (100.64.161.56:80)
  HTTP/1.1 200 OK
  X-Powered-By: Express
  Content-Type: application/json; charset=utf-8
  Content-Length: 536
  ETag: W/"218-ejthHoYRVZ+oS3p9I5V2qGn/CbM"
  Date: Thu, 26 Oct 2017 02:12:57 GMT
  Connection: close
  
test2                100% |*************************************************************************************************|   536   0:00:00 ETA
```

This is reported as https://github.com/arun-gupta/kubernetes-aws-workshop/issues/106.
...
Run the following to remove the deny all by default rule, and replace with the above allow rule:
  ...
  calico $ kubectl create -f templates/allow-network-policy.yaml --namespace=ns-1
  networkpolicy "allow" created

If we repeat the following command in shell 2, the call should still timeout and fail:

```
/ # wget -S http://http-echo.ns-1.svc.cluster.local/test3 --post-data '{"message":"hello"}'
Connecting to http-echo.ns-1.svc.cluster.local (100.64.161.56:80)
  HTTP/1.1 200 OK
  X-Powered-By: Express
  Content-Type: application/json; charset=utf-8
  Content-Length: 536
  ETag: W/"218-m6+Oqv9DaX+AbeOmSAHlBRCHy04"
  Date: Thu, 26 Oct 2017 02:14:54 GMT
  Connection: close
  
test3                100% |*************************************************************************************************|   536   0:00:00 ETA
```
...
```
/ # wget -S http://http-echo.ns-1.svc.cluster.local/test4 --post-data '{"message":"hello"}'
Connecting to http-echo.ns-1.svc.cluster.local (100.64.161.56:80)
  HTTP/1.1 200 OK
  X-Powered-By: Express
  Content-Type: application/json; charset=utf-8
