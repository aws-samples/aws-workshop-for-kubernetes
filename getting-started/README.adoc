...
We are using the VirtualBox driver which is the default selection for minikube, if you would like you can use an alternate supported component (xhyve driver or VMware Fusion) using the `--vm-driver=xxx` flag.

Start a single-node Kubernetes cluster on your local machine as:

    minikube start

First start of minikube will download the ISO file and then start the cluster. It shows the following output:
...
This is a convenient feature of minukube. But what if you wanted to find this information yourself?
...
IP address information can be obtained by looking for `InternalIP` field:
...
Look around the dashboard and become familiar with some the of Kubernetes terminology. This will be explained in the subsequent chapters.
