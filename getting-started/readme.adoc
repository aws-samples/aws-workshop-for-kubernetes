...
The output shows that there are two clusters. `NAME` column shows the context name. `*` indicates the current context.
...
Change the context to a different context.
  ...
  Switched to context "cluster.k8s.local".

In this case, the context is switched to a cluster that will be created in a later chapter.
