...
Other pods are shown as well, but we are only showing the etcd pods. Note down the name of the pod that starts with `etcd-server-ip`. Get more details about this pod:
...
Let's look at these these metrics in the Prometheus dashboard.
...
. Prometheus dashboard is accessible on port 30900. Edit the security group and add a rule to allow inbound traffic on port 30900 as shown:
+
